...
class DuDuAssistant(QWidget):
    def __init__(self, parent=None, **kwargs):
        super(DuDuAssistant, self).__init__(parent)
        self.init()
        self.initPall()
        self.initCondition()
        self.initPetImage()
    ...
    def initPetImage(self):
        ...
        self.chatState = []
        for i in os.listdir("./chat"):
            self.chatState.append("./chat/" + i)

        self.kissState = []
        for i in os.listdir("./kiss"):
            self.kissState.append("./kiss/" + i)

        self.hugState = []
        for i in os.listdir("./hug"):
            self.hugState.append("./hug/" + i)

        self.eatState = []
        for i in os.listdir("./eat"):
            self.eatState.append("./eat/" + i)

        self.beatState = []
        for i in os.listdir("./beat"):
            self.beatState.append("./beat/" + i)
    ...
    # 初始化托盘
    def initPall(self):
        icons = os.path.join('tigerIcon.png')
    ...
    def HighIntimacy(self):
        if self.intimacy > 50:
            self.randomIdle = []
            for i in os.listdir("./randomIdle"):
                self.randomIdle.append("./randomIdle/" + i)
            for i in os.listdir("./highIntimacy"):
                self.randomIdle.append("./highIntimacy/" + i)
        else:
            self.randomIdle = []
            for i in os.listdir("./randomIdle"):
                self.randomIdle.append("./randomIdle/" + i)
    ...
    def idleAction(self):
        self.movie = QMovie("./idle/idle.gif")
        # 宠物大小
    ...
    def clickState(self):
        self.movie = QMovie("./click/excited.gif")
        self.movie.setScaledSize(QSize(100, 100))
        self.image.setMovie(self.movie)
        self.movie.start()
    ...
    def contextMenuEvent(self, event):
        ...
        dudu = menu.addMenu("互动")
        kiss = dudu.addAction("贴贴")
        hug = dudu.addAction("抱抱")
        eat = dudu.addAction("吃好吃")
        wc = dudu.addAction("上厕所")
        beat = dudu.addAction("欺负")
        personal = dudu.addMenu("嘟嘟信息")
...
class ChatWindow(QWidget):
    ...
    def __init__(self):
        ...
        # 头像
        self.avatar1 = QPixmap("bubu.jpg")
        self.avatar2 = QPixmap("yier.jpg")
